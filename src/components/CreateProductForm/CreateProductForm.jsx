...
const CreateProductForm = ({ currentUser }) => {
    ...
    const fetchDetailsData = async () => {
        try {
            ...
            // const result1 = await axios.get('https://localhost:3001/products')
            const result1 = await axios.get('https://192.168.0.100:3001/products')
    ...
    const handleUpload = async () => {
        if (fileData) {
            ...
            // await axios.post(`https://localhost:3001/products/excel`, fileData);
            await axios.post(`https://192.168.0.100:3001/products/excel`, fileData);
            fileData = null
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (flagReduct === true) {
                setFlagReduct(false);
                // await axios.put(`https://localhost:3001/products/${selectedName}`, {nameToSend, savedDetails})
                await axios.put(`https://192.168.0.100:3001/products/${selectedName}`, {nameToSend, savedDetails})
            ...
            // await axios.post('https://localhost:3001/products', {nameToSend, savedDetails })
            await axios.post('https://192.168.0.100:3001/products', {nameToSend, savedDetails })
            localStorage.removeItem('selectedDetails');
            localStorage.removeItem('isEditMode');
            setName('');
            setProductDetails([]);
