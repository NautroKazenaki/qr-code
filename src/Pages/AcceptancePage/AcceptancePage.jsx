...
const AcceptancePage = ({userLevel}) => {
    ...
    useEffect(() => {
        const fetchProviders = async () => {
            try {
                // const response = await axios.get('https://localhost:3001/providers')
                const response = await axios.get('https://192.168.0.100:3001/providers')
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // const acceptanceResult = await axios.get('https://localhost:3001/acceptance')
                const acceptanceResult = await axios.get('https://192.168.0.100:3001/acceptance')
                debugger
                 setAcceptanceData(acceptanceResult);
                //  const usersResult = await axios.get('https://localhost:3001/users');
                 const usersResult = await axios.get('https://192.168.0.100:3001/users');
        ...
        if (trimmedName.length > 0) {
            if (!/^\s/.test(trimmedName)) {
                ...
                if (!isExistingProvider) {
                    try {
                        // await axios.post('https://localhost:3001/providers', {trimmedName})
                        await axios.post('https://192.168.0.100:3001/providers', {trimmedName})
                        // const result = await axios.get('https://localhost:3001/providers')
                        const result = await axios.get('https://192.168.0.100:3001/providers')
...
const handleRemoveProvider = async (providerNameToRemove) => {
    try {
        // await axios.delete(`https://localhost:3001/providers/${providerNameToRemove}`);
        await axios.delete(`https://192.168.0.100:3001/providers/${providerNameToRemove}`);
    ...
    const addAcceptanceToDB = async () => {
        try {
            ...
            for (const row of selectedRows) {
                ...
                try {
                    // const response = await axios.post('https://localhost:3001/acceptance', {
                    //     username,
                    //     currentDateTime,
                    //     name,
                    //     quantity,
                    ...
                    //     acceptanceCounter
                    // })
                    const response = await axios.post('https://192.168.0.100:3001/acceptance', {
                        username,
                        currentDateTime,
                        name,
                        quantity,
                ...
                try {
                    // const response = await axios.post('https://localhost:3001/details', {
                    //     name,
                    //     quantity,
                    //     selectedProvider,
                    // })
                    const response = await axios.post('https://192.168.0.100:3001/details', {
                        name,
                        quantity,
                        selectedProvider,
                    })
